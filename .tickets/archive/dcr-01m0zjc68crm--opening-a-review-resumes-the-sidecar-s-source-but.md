...
updated: '2026-08-26T18:08:29.742631884Z'
closed: '2026-08-26T18:08:29.742631884Z'
acceptance:
...
**2026-08-26T18:08:29.742631884Z**

revu--open reads the Sidecar before the caller's diff is taken: when the record and the caller's Source have parted, the recorded Source is read anew and the caller's diff is not taken at all, so the first paint and every later reload agree. The resume echo names both commits ("Resumed worktree-vs-qa against b689d65 (qa is now 394b1df)"); a staged Review never reports a parting, since git diff --cached is against HEAD whatever the record holds. Three tests in test/revu-review-test.el cover the branch-moved case, the echo, and the everyday worktree Review keeping the commit it recorded when HEAD moves under it; ADR-0005's worktree amendment records the rule, including that the worktree Review does not re-base on open. Commit 7ffb0d6.
